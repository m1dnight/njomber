defmodule NjomberWeb.CounterLive do
  ...
  def mount(_params, _session, socket) do
    socket =
      socket
      |> assign(:current_keypair, Njomber.default_keypair())
      |> assign(
        :keypair_form,
  ...
  def handle_event("create-keypair", _params, socket) do
    socket =
      socket
      |> assign(:current_keypair, Njomber.create_keypair())
  ...
  def handle_event("validate-keypair", %{"keypair" => keypair_params}, socket) do
    with {true, _} <- {valid_input?(keypair_params["nullifier_key"]), :nullifier_key},
         {true, _} <-
           {valid_input?(keypair_params["nullifier_key_commitment"]), :nullifier_key_commitment} do
      form = to_form(keypair_params, as: :keypair)
      {:noreply, assign(socket, :keypair_form, form)}
    else
      {_, key} ->
        form =
          to_form(keypair_params, as: :keypair, errors: [{key, {"Invalid hex", []}}])
  ...
  def handle_event("update-keypair", %{"keypair" => keypair_params}, socket) do
    keypair = %{
      nullifier_key: decode_key(keypair_params["nullifier_key"]),
      nullifier_key_commitment: decode_key(keypair_params["nullifier_key_commitment"])
    }
  ...
  defp valid_input?(key) do
    Base.decode64(key) != :error
  end

  def encode_key(bytes) do
    Base.encode64(Anoma.Util.binlist2bin(bytes))
  end

  def decode_key(hex) do
    iolist =
      hex
      |> Base.decode64!()
